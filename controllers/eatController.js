...
router.get("/", (req, res) => {
    ...
    fetch(url, requestOptions)
        ...
        .then(result => Promise.resolve(result.response.groups[0].items.map(data => {
            ...
            delete venue.photos;
            venue.thumbnails = `${process.env.APP_URL}/api/eats/${venue.id}/thumbnails`
            return venue;
        })))
...
router.get("/:id", (req, res) => {
    ...
    fetch(`https://api.foursquare.com/v2/venues/${req.params.id}?client_id=${process.env.FOURSQUARE_CLIENT_ID}&client_secret=${process.env.FOURSQUARE_CLIENT_SECRET}&v=${process.env.FOURSQUARE_V}`, requestOptions)
        .then(response => response.json())
